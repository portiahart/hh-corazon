...
export default function LoginPage() {
  ...
  return (
    <div style={styles.page}>
      ...
      {/* Hero buttons — visible before login */}
      <div style={styles.heroRow}>
        <a href="#" style={{ ...styles.heroBtn, background: '#FFD000', color: '#1F2D3D', border: '2.5px solid #1F2D3D' }}>
          <ClockIcon size={40} />
          <span>MARCAR ENTRADA / SALIDA</span>
        </a>
        <a href="https://cash.portiahart.com" style={{ ...styles.heroBtn, background: '#1F2D3D', color: '#FFD000', border: '2.5px solid #1F2D3D' }}>
          <DollarIcon size={40} />
          <span>CASH APP</span>
        </a>
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  logo: { height: 60, width: 'auto', marginBottom: 28 },
  heroRow: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, width: '100%' },
  heroBtn: {
    display: 'flex', flexDirection: 'column', alignItems: 'center',
    justifyContent: 'center', textAlign: 'center', textDecoration: 'none',
    borderRadius: 14, padding: '28px 16px', gap: 16,
    fontSize: '0.66rem', fontWeight: 500, letterSpacing: '0.14em',
    lineHeight: 1.4,
  },
  form: {
    width: '100%',
  ...
  formLabel: {
    ...
    color: '#7A91A5', marginBottom: 4,
  },
  input: {
    padding: '12px 14px', borderRadius: 8, border: '1.5px solid rgba(31,45,61,0.15)',
    fontSize: '0.875rem', fontFamily: 'inherit', background: '#F2F5F8',
    outline: 'none', color: '#1F2D3D',
  },
  error: { fontSize: '0.75rem', color: '#FC0083' },
  submitBtn: {
    padding: '13px', borderRadius: 8, background: '#1F2D3D', color: '#FFD000',
